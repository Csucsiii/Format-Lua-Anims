...
function jsObjToLuaTable(jsObj, indentLevel = 0) {
    ...
    for (const key in jsObj) {
        if (jsObj.hasOwnProperty(key)) {
            ...
            luaTable += `${indent}    ${key} = ${toLuaValue(value, indentLevel + 1)}`;
        }
    }
...
function toLuaValue(value, indentLevel) {
...
const main = async () => {
    if (!fs.existsSync(INPUT_FOLDER_PATH)){
        ...
        console.log("A(z) 'input' mappa létrehozva, másold be ide a(z) anims.json fájlt.");
        return;
    };
    ...
    try{
        ...
        fs.writeFileSync(OUTPUT_LUA_PATH, `local anims = ${jsObjToLuaTable(formattedAnims)}`);

        console.log(`Sikeresen átformáztad a fájlokat! Elérési út: ${OUTPUT_JSON_PATH}`);
    }catch(err){
